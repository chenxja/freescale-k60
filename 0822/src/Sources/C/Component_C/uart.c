...
{
	unsigned long uartclk_khz=45000;//时钟180MHz
	unsigned long baud=115200;//波特率256000
	unsigned short sbr,brfa;
	PORTD_PCR8|=PORT_PCR_MUX(3);//将D6引脚设置为模式3，即UART0_RX
	PORTD_PCR9|=PORT_PCR_MUX(3);//将D7引脚设置为模式3，即UART0_TX
	SIM_SCGC1|=SIM_SCGC1_UART5_MASK;//开启UART0时钟
	sbr=(unsigned short)((uartclk_khz*1000)/(baud*16));//计算并设置波特率
	
	UART5_BDH=(unsigned char)((sbr&0x1F00)>>8);
	UART5_BDL=(unsigned char)(sbr&0x00FF);
